...
export default function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: profile } = await supabase
        .from('profiles').select('family_id, full_name').eq('id', user.id).single();
      if (!profile?.family_id) {
        setMembers([{ id: user.id, full_name: profile?.full_name || user.email?.split('@')[0] || 'Я', role: 'admin' }]);
        return;
      }
      ...
      const { data: mems } = await supabase.from('profiles').select('id, full_name, role').eq('family_id', profile.family_id);
      setMembers(mems || []);
    };
    load();
  }, []);
  ...
  return (
    <aside
      ...
      style={{ position: 'fixed', top: 0, left: 0, height: '100vh', width: 230, background: 'linear-gradient(180deg,#1E2F52 0%,#152035 100%)', boxShadow: '2px 0 16px rgba(0,0,0,.12)', display: 'flex', flexDirection: 'column', overflowY: 'auto', zIndex: 40 }}>

      {/* Лого + кнопка закрити на мобільному */}
      <div style={{ padding: '22px 18px 18px', borderBottom: '1px solid rgba(255,255,255,.08)', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {onClose && (
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,.5)', fontSize: 18, cursor: 'pointer', padding: 4 }}>✕</button>
        )}
      </div>
        ...
        {nav.map(({ section, items }) => (
          <div key={section}>
            ...
            {items.map(item => {
              ...
              return (
                <Link key={item.href} href={item.href} onClick={onClose} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 18px', fontSize: 13, borderLeft: active ? '3px solid #1EB788' : '3px solid transparent', background: active ? 'rgba(30,183,136,.13)' : 'transparent', color: active ? '#1EB788' : 'rgba(255,255,255,.6)', fontWeight: active ? 700 : 400, borderRadius: active ? '0 8px 8px 0' : 0, textDecoration: 'none', transition: 'all .15s', marginRight: active ? 10 : 0 }}>
