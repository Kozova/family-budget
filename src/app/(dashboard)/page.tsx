...
export default function CategoriesPage() {
  ...
  const load = async () => {
    ...
    const { data: lims } = await supabase
      .from("category_limits").select("category_name, monthly_limit").eq("user_id", user.id);
    ...
    return (
      <div key={catName} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid #F5F5F8" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {limit ? (
            <div style={{ fontSize: 11, color: "#1EB788", marginTop: 2, fontWeight: 600 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {isDefault && (
            <div style={{ position: "relative", display: "inline-block" }}
              title="Стандартна категорія — назву не можна змінити, але можна встановити ліміт">
              <button
                onClick={() => openEdit(catName, catEmoji, catType, true)}
                style={{ background: "#F0F2F5", border: "none", borderRadius: 7, padding: "5px 10px", fontSize: 12, cursor: "pointer", color: "#6B7280" }}>
                {limit ? "✏️ Ліміт" : "＋ Ліміт"}
              </button>
            </div>
          )}
  ...
  const formTitle = editState?.isDefault
    ? `Встановити ліміт: ${editState.name}`
    : editState?.catId ? `Редагувати: ${editState.name}` : "Нова категорія";
    ...
    <div>
      ...
      {showForm && (
        <div style={{ background: "#fff", borderRadius: 16, padding: 24, border: "1px solid #E8EAF0", marginBottom: 16, boxShadow: "0 2px 8px rgba(26,39,68,.06)" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#1A2744", marginBottom: 16 }}>{formTitle}</div>
          ...
          {(type === "expense" || editState?.isDefault) && (
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="number" value={limitAmount} onChange={e => setLimitAmount(e.target.value)} placeholder="Наприклад: 5000" style={{ width: "100%", border: "1.5px solid #E8EAF0", borderRadius: 8, padding: "9px 12px", fontSize: 13, outline: "none", color: "#1A2744" }} />
