...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      data.filter(t => t.type === "expense").forEach(t => {
        catMap[t.description] = (catMap[t.description] || 0) + Number(t.amount);
      });
  ...
  if (loading) {
    return <div style={{ padding: 40, textAlign: "center", color: "#9CA3AF" }}>Завантаження...</div>;
  }
    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: 20, border: "1px solid #E8EAF0", boxShadow: "0 2px 8px rgba(26,39,68,.06)" }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 700, color: "#1EB788" }}>
            {income.toLocaleString("uk-UA")} грн
          </div>
          <div style={{ marginTop: 10, height: 8, background: "#F0F2F5", borderRadius: 4 }}>
            <div style={{ height: "100%", width: String(income / total * 100) + "%", background: "#1EB788", borderRadius: 4 }} />
          </div>
        </div>
        <div style={{ background: "#fff", borderRadius: 16, padding: 20, border: "1px solid #E8EAF0", boxShadow: "0 2px 8px rgba(26,39,68,.06)" }}>
          <div style={{ fontSize: 11, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 6 }}>Витрати цього місяця</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: "#E24B4A" }}>
            {expense.toLocaleString("uk-UA")} грн
          </div>
          <div style={{ marginTop: 10, height: 8, background: "#F0F2F5", borderRadius: 4 }}>
            <div style={{ height: "100%", width: String(expense / total * 100) + "%", background: "#E24B4A", borderRadius: 4 }} />
          </div>
        </div>
      </div>
          ...
          cats.map((cat, i) => (
            <div key={cat.name} style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 5 }}>
                ...
                <span style={{ color: "#E24B4A", fontWeight: 700 }}>{cat.amount.toLocaleString("uk-UA")} грн</span>
              </div>
              <div style={{ height: 7, background: "#F0F2F5", borderRadius: 4 }}>
                <div style={{ height: "100%", width: String(cat.amount / maxCat * 100) + "%", background: COLORS[i % 5], borderRadius: 4 }} />
              </div>
            </div>
          ))
        )}
