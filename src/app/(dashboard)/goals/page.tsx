...
export default function GoalsPage() {
  ...
  const handleAdd = async (id: string, current: number, add: number) => {
    await supabase.from("goals").update({ current_amount: current + add }).eq("id", id);
    load();
  };
          ...
          return (
            <div key={g.id} style={{ background: "#fff", borderRadius: 16, padding: 20, border: "1px solid #E8EAF0", boxShadow: "0 2px 8px rgba(26,39,68,.06)", marginBottom: 13 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                <div style={{ width: 46, height: 46, borderRadius: 12, background: "#F0FBF7", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>{g.emoji}</div>
                <div style={{ flex: 1 }}>
                ...
                <button onClick={() => handleDelete(g.id)} style={{ background: "none", border: "1.5px solid #E8EAF0", borderRadius: 7, width: 30, height: 30, cursor: "pointer", fontSize: 14, color: "#9CA3AF" }}>🗑️</button>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#6B7280", marginBottom: 9 }}>
                <span>Накопичено: <strong style={{ color: "#1A2744" }}>{g.current_amount.toLocaleString("uk-UA")} грн</strong></span>
                <span>Ціль: <strong style={{ color: "#1A2744" }}>{g.target_amount.toLocaleString("uk-UA")} грн</strong></span>
                <span>Залишилось: <strong style={{ color: "#1A2744" }}>{left.toLocaleString("uk-UA")} грн</strong></span>
              </div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {[1000, 5000, 10000].map(amt => (
                  <button key={amt} onClick={() => handleAdd(g.id, g.current_amount, amt)} style={{ border: "1.5px solid #E8EAF0", background: "#F9FAFB", borderRadius: 8, padding: "5px 12px", fontSize: 12, cursor: "pointer", color: "#1A2744", fontWeight: 500 }}>
